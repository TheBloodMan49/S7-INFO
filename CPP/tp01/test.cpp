...
int main(int argc, char *argv[]) {
    ...
    string copie_tpc(
            tpc); // constructeur par copie (se génère tout seul mais ici il sera défecteux, il faut le réécrire explicitement)
    string progname(argv[0]);
    string *pcopie_tpc = new string(
            "tp de c++"); //on fait de l'allocation dynamique, bien sûr c'est un peu ridicule dans ce cas de figure
    ...
    std::cout << "chaine tpc:\"" << tpc << "\"  doit avoir une taille de 9:" << eval(true, tpc.size() == 9) << endl;
    std::cout << "chaine copie_tpc:\"" << copie_tpc << "\" doit avoir une taille de 9:"
              << eval(true, copie_tpc.size() == 9) << endl;
    std::cout << "chaine pcopie_tpc:\"" << vide << "\"  doit avoir une taille de 9:"
              << eval(true, pcopie_tpc->size() == 9) << endl;
    std::cout << "chaine progname:\"" << progname << "\"" << endl;

    //test getters et setters simples
    std::cout << "le second caractère de la chaine " << TPC << " est P -> " << TPC[1] << " "
              << eval(true, TPC[1] == 'P') << endl;
    progname[0] = 'F';
    std::cout << "le premier caractère de la chaine " << progname << " est désormais F -> " << progname[0] << " "
              << eval(true, progname[0] == 'F') << endl;
    ...
    string vole_tpc(std::move(
            tpc)); //illustration de la "sémantique de déplacement" vole_tpc s'approprie le contenu de tpc ici qui est considéré mort après
